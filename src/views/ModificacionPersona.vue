<template>
    <v-container>
        <v-card elevation="12">
            <v-card-title class="blue lighten-5 justify-center"><strong>MODIFICACION DE PERSONA</strong></v-card-title>
            <v-card-text>
                <v-form v-model="validoFormulario" ref="formularioRegistro">
                    ...
                    <v-row>
                        <v-col>
                            ...
                            v-model="personaAModificar.celular"
                            :rules="reglaCampoVacio"
                            type="number"></v-text-field>
                        </v-col>
                        <v-col>
                        ...
                        <v-col>
                            ...
                            v-model="personaAModificar.emailInstitucional"></v-text-field>
                        </v-col>
                    </v-row>
                    ...
                    <v-row>
                        <v-col>
                            ...
                            :rules="reglaCampoVacio"
                            v-model="personaAModificar.tipoInvestigador"></v-select>
                        </v-col>
                        <v-col>
                            <v-select v-show="personaAModificar.tipoInvestigador != null" v-if="personaAModificar.tipoInvestigador == 'UTN'"
                            label="Categoría de investigador"
                            :items="categoriasUTN"
                            v-model="personaAModificar.categoriaInvestigador">
                            </v-select>
                            <v-select v-show="personaAModificar.tipoInvestigador != null" v-if="personaAModificar.tipoInvestigador == 'Nacional'"
                            label="Categoría de investigador"
                            :items="categoriasNacional"
                            v-model="personaAModificar.categoriaInvestigador">
                            </v-select>
                            <v-select v-show="personaAModificar.tipoInvestigador != null" v-if="personaAModificar.tipoInvestigador == 'CONICET'"
                            label="Categoría de investigador"
                            :items="categoriasCONICET"
                            v-model="personaAModificar.categoriaInvestigador">
                            </v-select>
                            <v-select v-show="personaAModificar.tipoInvestigador != null" v-if="personaAModificar.tipoInvestigador == 'CIC'"
                            label="Categoría de investigador"
                            :items="categoriasCIC"
                            v-model="personaAModificar.categoriaInvestigador">
                            </v-select>
                        ...
                        <v-col>
                            <v-text-field
                            v-show="personaAModificar.tipoInvestigador != null"
                            label="Número de resolución de la categoría"
                            hint="Ingrese el número de resolución"
                            v-model="personaAModificar.numeroResolucion"
                            type="number"></v-text-field>
                        </v-col>
                        <v-col>
                            <v-menu
                            ...
                            :return-value.sync="personaAModificar.fechaObtencionCategoria"
                            ...
                            >
                                <template v-slot:activator="{ on, attrs }">
                                    <v-text-field
                                    v-show="personaAModificar.tipoInvestigador != null"
                                    v-model="personaAModificar.fechaObtencionCategoria"
                                ...
                                <v-date-picker v-model="personaAModificar.fechaObtencionCategoria" no-title scrollable>
                                <v-spacer></v-spacer>
                                <v-btn text color="deep-orange darken-4" @click="menu = false">Cancelar</v-btn>
                                <v-btn text color="success" @click="$refs.menu.save(personaAModificar.fechaObtencionCategoria)">OK</v-btn>
                                </v-date-picker>
                            </v-menu>
                        </v-col>
                    </v-row>
                    <v-row>
                        ...
                        <v-col>
                            <v-select v-if="personaAModificar.situacionAcademica == 'Alumno'"
                            label="Ingrese cantidad de horas semanales dedicadas a investigación*"
                            :items="horasAlumno"
                            v-model="personaAModificar.horaSemanal"
                            :rules="reglaCampoVacio"
                            v-show="personaAModificar.situacionAcademica != null">
                            </v-select>
                            <v-select v-if="personaAModificar.situacionAcademica == 'Graduado'"
                            label="Ingrese cantidad de horas semanales dedicadas a investigación*"
                            :items="horasGraduado"
                            v-model="personaAModificar.horaSemanal"
                            :rules="reglaCampoVacio"
                            ...
                            <v-select v-if="personaAModificar.situacionAcademica == 'Docente'"
                            label="Ingrese cantidad de horas semanales dedicadas a investigación*"
                            :items="horasDocente"
                            v-model="personaAModificar.horaSemanal"
                            :rules="reglaCampoVacio"
                            v-show="personaAModificar.situacionAcademica != null"></v-select>
                        </v-col>
                        <v-col>
                            <v-btn v-show="personaAModificar.situacionAcademica == 'Docente'" block @click="mostrarDialogMateria">Cargar materia en la que participa</v-btn>
                        </v-col>
                        <v-col>
                            <v-btn v-show="personaAModificar.situacionAcademica == 'Docente'" block @click="mostrarListaMaterias">Mostrar materias agregadas</v-btn>
                        </v-col>
                    ...
                    <v-row>
                        <v-col>
                            <v-row>
                                ...
                                <v-col>
                                    <v-btn color="primary" v-model="personaAModificar.pasaporte.tiene" rounded>No posee pasaporte</v-btn>
                    ...
                    <v-row>
                        ...
                        <v-col>
                            <v-row>
                                <v-col v-if="personaAModificar.pasaporte.tiene == 'Si'">
                    ...
                    <v-form v-model="validoPasaporte" ref="formularioPasaporte">
                        <v-row>
                            ...
                            <v-col>
                                ...
                                v-model="menu2"
                                :close-on-content-click="false"
                                :return-value.sync="personaAModificar.pasaporte.fechaVencimiento"
                                transition="scale-transition"
                                offset-y
                                min-width="290px">
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-text-field
                                        v-model="personaAModificar.pasaporte.fechaVencimiento"
                                    ...
                                    <v-date-picker v-model="personaAModificar.pasaporte.fechaVencimiento" no-title scrollable>
                                    <v-spacer></v-spacer>
                                    <v-btn text color="deep-orange darken-4" @click="menu2 = false">Cancelar</v-btn>
                                    <v-btn text color="success" @click="$refs.menu2.save(personaAModificar.pasaporte.fechaVencimiento)">OK</v-btn>
                                    </v-date-picker>
                                </v-menu>
                            </v-col>
                        </v-row>
