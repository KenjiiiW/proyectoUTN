    ...
    <v-container>
        <v-card>
            <v-card-title style="background-color:#EEEEEE" class="justify-center titulo">
                <strong>PRÁCTICAS PROFESIONALIZANTES DE SECUNDARIA</strong>
                <v-spacer></v-spacer>
                <v-text-field
                    v-model="search"
                ...
                ></v-text-field>
                <v-spacer></v-spacer>
                <v-btn dark color="light-blue accent-3" elevation="12" fab to="AltaPracticaProfesionalizanteSecundaria"><v-icon>add</v-icon></v-btn>
            </v-card-title>
            <v-data-table
            :headers="headers"
            :items="item"
        ...
        <v-dialog max-width="25%" v-model="dialogEliminar">
            <v-card>
                <v-card-title class="justify-center">
                    ¿Seguro desea eliminar esta práctica?
                </v-card-title>
                <v-card-actions>
                    <v-spacer>
